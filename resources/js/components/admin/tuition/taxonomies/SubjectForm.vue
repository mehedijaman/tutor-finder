...
const defaultClassId = props.initial.class_id ?? props.schoolClasses[0]?.id ?? null;
...
const classLabel = computed(() => {
    const selected = props.schoolClasses.find((item) => item.id === Number(form.class_id));

    if (!selected) {
        return '';
    }

    return selected.category_name ? `${selected.name} (${selected.category_name})` : selected.name;
});
    ...
    <form class="space-y-6" @submit.prevent="submit">
        <section class="grid gap-4 rounded-xl border bg-white p-4">
            ...
            <div class="grid gap-2">
                ...
                <select id="subject-class" v-model="form.class_id" class="h-10 rounded-md border px-3 text-sm" required>
                ...
                <p v-if="classLabel" class="text-xs text-muted-foreground">Selected: {{ classLabel }}</p>
                <InputError :message="form.errors.class_id" />
            </div>

            <div class="grid gap-2">
                <Label for="subject-name">Name</Label>
                <Input id="subject-name" v-model="form.name" type="text" required />
            ...
            <div class="grid gap-2">
                ...
                <Input id="subject-slug" :model-value="form.slug" type="text" @update:model-value="onManualSlugInput" />
            ...
            <div class="grid gap-2 sm:grid-cols-2">
                <div class="grid gap-2">
                    ...
                    <select id="subject-status" v-model="form.status" class="h-10 rounded-md border px-3 text-sm">
                ...
                <div class="grid gap-2">
                    ...
                    <Input id="subject-sort-order" v-model.number="form.sort_order" type="number" min="0" />

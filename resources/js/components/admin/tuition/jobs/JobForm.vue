...
const { autoSlug, onManualSlugInput, toggleAutoSlug } = useAutoSlug(toRef(form, 'title'), toRef(form, 'slug'), {
...
const filteredClasses = computed(() => {
    ...
    return props.schoolClasses.filter((schoolClass) => Number(schoolClass.category_id) === categoryId);
});
...
const filteredSubjects = computed(() => {
    ...
    return props.subjects.filter((subject) => Number(subject.class_id) === classId);
});
...
watch(
    ...
    () => {
        ...
        form.subject_ids = (form.subject_ids ?? []).filter((subjectId) => subjectIds.includes(Number(subjectId)));
    },
);
...
function submit() {
    ...
    form.transform(transformPayload).post(props.action, { preserveScroll: true });
}
</script>
    ...
    <form class="space-y-6" @submit.prevent="submit">
        <section class="grid gap-4 rounded-xl border bg-white p-4">
            ...
            <div class="grid gap-2">
                ...
                <Input id="job-title" v-model="form.title" type="text" required />
            ...
            <div class="grid gap-2">
                ...
                <Input id="job-slug" :model-value="form.slug" type="text" @update:model-value="onManualSlugInput" />
                <InputError :message="form.errors.slug" />
            </div>

            <div class="grid gap-2">
                <Label for="job-description">Description</Label>
                <textarea id="job-description" v-model="form.description" class="min-h-36 rounded-md border px-3 py-2 text-sm" required />
                <InputError :message="form.errors.description" />
            </div>
        </section>

        <section class="grid gap-4 rounded-xl border bg-white p-4 lg:grid-cols-2">
            <h2 class="text-lg font-semibold lg:col-span-2">Taxonomies and Location</h2>

            <div class="grid gap-2">
                <Label for="job-tuition-type">Tuition Type</Label>
                <select id="job-tuition-type" v-model="form.tuition_type_id" class="h-10 rounded-md border px-3 text-sm" required>
                    <option value="">Select tuition type</option>
                    <option v-for="item in tuitionTypes" :key="item.id" :value="item.id">{{ item.name }}</option>
                </select>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-category" v-model="form.category_id" class="h-10 rounded-md border px-3 text-sm" required>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-class" v-model="form.class_id" class="h-10 rounded-md border px-3 text-sm" required>
                    <option value="">Select class</option>
                    <option v-for="item in filteredClasses" :key="item.id" :value="item.id">{{ item.name }}</option>
                </select>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-country" v-model="form.country_id" class="h-10 rounded-md border px-3 text-sm" required>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-city" v-model="form.city_id" class="h-10 rounded-md border px-3 text-sm" required>
                    <option value="">Select city</option>
                    <option v-for="item in filteredCities" :key="item.id" :value="item.id">{{ item.name }}</option>
                </select>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-area" v-model="form.area_id" class="h-10 rounded-md border px-3 text-sm">
                    <option value="">Select area (optional)</option>
                    <option v-for="item in filteredAreas" :key="item.id" :value="item.id">{{ item.name }}</option>
                </select>
            ...
            <div class="grid gap-2">
                ...
                <Input id="job-latitude" v-model="form.latitude" type="number" step="0.00000001" />
                <InputError :message="form.errors.latitude" />
            </div>

            <div class="grid gap-2">
                <Label for="job-longitude">Longitude</Label>
                <Input id="job-longitude" v-model="form.longitude" type="number" step="0.00000001" />
                <InputError :message="form.errors.longitude" />
            </div>

            <div class="grid gap-2 lg:col-span-2">
                <Label>Subjects (select at least one)</Label>
                <div class="grid gap-2 rounded-md border p-3 md:grid-cols-2 lg:grid-cols-3">
                    <label v-for="subject in filteredSubjects" :key="subject.id" class="flex items-center gap-2 text-sm">
                        <input v-model="form.subject_ids" type="checkbox" :value="subject.id" class="h-4 w-4" />
                        <span>{{ subject.name }}</span>
                    </label>

                    <p v-if="filteredSubjects.length === 0" class="text-xs text-muted-foreground">No subjects available for the selected class.</p>
                </div>
        ...
        <section class="grid gap-4 rounded-xl border bg-white p-4 lg:grid-cols-2">
            <h2 class="text-lg font-semibold lg:col-span-2">Schedule and Preferences</h2>

            <div class="grid gap-2">
                <Label for="job-student-gender">Student Gender</Label>
                <select id="job-student-gender" v-model="form.student_gender" class="h-10 rounded-md border px-3 text-sm" required>
            ...
            <div class="grid gap-2">
                ...
                <select id="job-tutor-gender" v-model="form.tutor_gender" class="h-10 rounded-md border px-3 text-sm" required>
                    <option v-for="option in genderOptions" :key="option.value" :value="option.value">{{ option.label }}</option>
                </select>
            ...
            <div class="grid gap-2 lg:col-span-2">
                ...
                <div class="grid gap-2 rounded-md border p-3 md:grid-cols-3 lg:grid-cols-4">
                    <label v-for="day in dayOptions" :key="day.value" class="flex items-center gap-2 text-sm">
                        <input v-model="form.tuition_days" type="checkbox" :value="day.value" class="h-4 w-4" />
                        <span>{{ day.label }}</span>
                    </label>
                </div>
            ...
            <div class="grid gap-2">
                ...
                <Input id="job-tuition-time" v-model="form.tuition_time" type="text" placeholder="e.g. 5:00 PM - 7:00 PM" />
                <InputError :message="form.errors.tuition_time" />
            </div>

            <div class="grid gap-2">
                <Label for="job-tuition-duration">Tuition Duration</Label>
                <Input id="job-tuition-duration" v-model="form.tuition_duration" type="text" placeholder="e.g. 3 months" />
                <InputError :message="form.errors.tuition_duration" />
            </div>

            <div class="grid gap-2">
                <Label for="job-students">No. of Students</Label>
                <Input id="job-students" v-model="form.no_of_students" type="number" min="1" />
                <InputError :message="form.errors.no_of_students" />
            </div>

            <div class="grid gap-2">
                <Label for="job-expires-at">Expires At</Label>
                <Input id="job-expires-at" v-model="form.expires_at" type="datetime-local" />
                <InputError :message="form.errors.expires_at" />
            </div>
        </section>

        <section class="grid gap-4 rounded-xl border bg-white p-4 lg:grid-cols-2">
            <h2 class="text-lg font-semibold lg:col-span-2">Salary and Publishing</h2>

            <div class="grid gap-2">
                <Label for="job-salary-amount">Salary Amount</Label>
                <Input id="job-salary-amount" v-model="form.salary_amount" type="number" min="0" step="0.01" />
                <InputError :message="form.errors.salary_amount" />
            </div>

            <div class="grid gap-2">
                <Label for="job-salary-currency">Salary Currency</Label>
                <Input id="job-salary-currency" v-model="form.salary_currency" type="text" />
                <InputError :message="form.errors.salary_currency" />
            </div>

            <label class="flex items-center gap-2 text-sm lg:col-span-2">
                <input v-model="form.salary_negotiable" type="checkbox" class="h-4 w-4" />
            ...
            <template v-if="isAdmin">
                <div class="grid gap-2">
                    ...
                    <select id="job-guardian" v-model="form.guardian_id" class="h-10 rounded-md border px-3 text-sm" required>
                ...
                <div class="grid gap-2">
                    ...
                    <select id="job-status" v-model="form.status" class="h-10 rounded-md border px-3 text-sm" required>
                ...
                <div class="grid gap-2 lg:col-span-2">
                    <Label for="job-published-at">Published At (optional)</Label>
                    <Input id="job-published-at" v-model="form.published_at" type="datetime-local" />

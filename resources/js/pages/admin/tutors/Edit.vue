            ...
            <Form :action="`/admin/tutors/${tutor.id}`" method="put" class="space-y-4" #default="{ errors, processing }">
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="grid gap-2">
                        ...
                        <Input id="name" name="name" type="text" :value="tutor.name" required />
                        <InputError :message="errors.name" />
                    </div>

                    <div class="grid gap-2">
                        <Label for="phone">Phone</Label>
                        <Input id="phone" name="phone" type="text" :value="tutor.phone || ''" />
                        <InputError :message="errors.phone" />
                    </div>

                    <div class="grid gap-2">
                        <Label for="email">Email</Label>
                        <Input id="email" name="email" type="email" :value="tutor.email || ''" />
                        <InputError :message="errors.email" />
                    </div>

                    <div class="grid gap-2">
                        <Label for="status">Status</Label>
                        <select id="status" name="status" :value="tutor.status" class="h-10 rounded-md border px-3 text-sm">

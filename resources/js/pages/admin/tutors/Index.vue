...
const breadcrumbs = [{ title: 'Tutors', href: '/admin/tutors' }];
...
function openConfirm(action, row = null) {
    ...
    if (action === 'delete') {
        ...
        confirmDescription.value = 'This will move the tutor to recycle bin.';
        confirmLabel.value = 'Delete';
        confirmDestructive.value = true;
    }

    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete Tutor';
        confirmDescription.value = 'This action is irreversible and removes the tutor permanently.';
    ...
    if (action === 'empty-recycle-bin') {
        ...
        confirmDescription.value = 'This will permanently delete all trashed tutors.';
        confirmLabel.value = 'Empty Recycle Bin';
        confirmDestructive.value = true;
    }

    if (action === 'restore') {
        confirmTitle.value = 'Restore Tutor';
        confirmDescription.value = 'This will restore the tutor from recycle bin.';
        confirmLabel.value = 'Restore';
    }

    if (action === 'restore-all') {
        confirmTitle.value = 'Restore All Tutors';
        confirmDescription.value = 'This will restore all trashed tutors.';
        confirmLabel.value = 'Restore All';
    }

    if (action === 'suspend') {
        confirmTitle.value = 'Suspend Tutor';
    ...
    if (action === 'unsuspend') {
        confirmTitle.value = 'Unsuspend Tutor';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/tutors/${row.id}`);
    }

    if (action === 'force-delete' && row) {
        router.delete(`/admin/tutors/${row.id}/force`);
    }

    if (action === 'restore' && row) {
        router.patch(`/admin/tutors/${row.id}/restore`);
    }
    ...
    if (action === 'suspend' && row) {
        router.patch(`/admin/tutors/${row.id}/status`, { status: 'suspended' });
    }

    if (action === 'unsuspend' && row) {
        router.patch(`/admin/tutors/${row.id}/status`, { status: 'active' });
    }
...
function handleRowAction(actionKey, row) {
    ...
    if (actionKey === 'edit') {
        router.visit(`/admin/tutors/${row.id}/edit`);
        return;
    }
        ...
        <div class="space-y-4 p-6">
            ...
            <div class="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-2">
                <div class="grid gap-2">
                    <Label for="tutor-search">Search</Label>
                    <Input
                        id="tutor-search"
                        v-model="search"
                        type="text"

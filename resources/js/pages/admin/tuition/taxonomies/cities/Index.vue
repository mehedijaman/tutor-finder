...
const breadcrumbs = [{ title: 'Cities', href: '/admin/tuition/taxonomies/cities' }];
...
const countryFilter = ref(props.filters.country_id ? String(props.filters.country_id) : 'all');
...
watch(() => props.filters.country_id, (value) => {
...
function applyFilters(overrides = {}) {
    router.get(
        baseUrl,
        {
            ...
            country_id: countryFilter.value === 'all' ? '' : countryFilter.value,
            ...overrides,
        },
        {
            preserveState: true,
...
function openConfirm(action, row = null, payload = {}) {
    ...
    if (action === 'status') {
        ...
        confirmDescription.value = isDeactivate ? 'This city will become inactive.' : 'This city will become active.';
        confirmLabel.value = isDeactivate ? 'Deactivate' : 'Activate';
    }

    if (action === 'restore') {
        confirmTitle.value = 'Restore City';
        confirmDescription.value = 'This will restore the city from recycle bin.';
    ...
    if (action === 'empty-recycle-bin') {
        ...
        confirmDescription.value = 'This will permanently remove all eligible trashed cities.';
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="space-y-1">
                    <h1 class="text-2xl font-semibold">{{ filters.trash ? 'City Recycle Bin' : 'Cities' }}</h1>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Link v-if="!filters.trash" href="/admin/tuition/taxonomies/cities/create" class="rounded-md bg-black px-4 py-2 text-sm text-white">
                        Create City
                    </Link>
                </div>
            </div>
            ...
            <DataTable :items="items" :columns="columns" empty-text="No cities found.">

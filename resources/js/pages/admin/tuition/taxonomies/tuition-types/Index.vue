...
function openConfirm(action, row = null, payload = {}) {
    ...
    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete Tuition Type';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/tuition/taxonomies/tuition-types/${row.id}`);
    }
...
function handleRowAction(actionKey, row) {
    if (actionKey === 'edit') {
        router.visit(`/admin/tuition/taxonomies/tuition-types/${row.id}/edit`);
...
<template>
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-4 sm:p-6 lg:p-8">
            ...
            <div
                class="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-2 lg:grid-cols-3 dark:border-slate-800 dark:bg-slate-900"
            >
                <Input
                    v-model="search"
                    type="text"

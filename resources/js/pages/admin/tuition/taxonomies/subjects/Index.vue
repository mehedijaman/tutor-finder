...
const breadcrumbs = [{ title: 'Subjects', href: '/admin/tuition/taxonomies/subjects' }];
const baseUrl = '/admin/tuition/taxonomies/subjects';
...
const classFilter = ref(props.filters.class_id ? String(props.filters.class_id) : 'all');
...
watch(() => props.filters.class_id, (value) => {
    const normalized = value ? String(value) : 'all';

    if (normalized !== classFilter.value) {
        classFilter.value = normalized;
    }
});
...
watch(classFilter, (value) => {
    applyFilters({ class_id: value === 'all' ? '' : value, page: 1 });
});
...
function applyFilters(overrides = {}) {
    router.get(
        baseUrl,
        {
            ...
            class_id: classFilter.value === 'all' ? '' : classFilter.value,
            ...overrides,
        },
        {
            preserveState: true,
...
function openConfirm(action, row = null, payload = {}) {
    ...
    if (action === 'delete') {
        confirmTitle.value = 'Delete Subject';
        confirmDescription.value = 'This will move the subject to recycle bin.';
    ...
    if (action === 'status') {
        ...
        confirmTitle.value = isDeactivate ? 'Deactivate Subject' : 'Activate Subject';
        confirmDescription.value = isDeactivate ? 'This subject will become inactive.' : 'This subject will become active.';
        confirmLabel.value = isDeactivate ? 'Deactivate' : 'Activate';
    }

    if (action === 'restore') {
        confirmTitle.value = 'Restore Subject';
        confirmDescription.value = 'This will restore the subject from recycle bin.';
        confirmLabel.value = 'Restore';
    }

    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete Subject';
    ...
    if (action === 'empty-recycle-bin') {
        ...
        confirmDescription.value = 'This will permanently remove all trashed subjects.';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/tuition/taxonomies/subjects/${row.id}`);
    }

    if (action === 'status' && row) {
        router.patch(`/admin/tuition/taxonomies/subjects/${row.id}/status`, {
            status: payload.status,
        });
    }

    if (action === 'restore' && row) {
        router.patch(`/admin/tuition/taxonomies/subjects/${row.id}/restore`);
    }

    if (action === 'force-delete' && row) {
        router.delete(`/admin/tuition/taxonomies/subjects/${row.id}/force`);
    }

    if (action === 'empty-recycle-bin') {
        router.delete('/admin/tuition/taxonomies/subjects/recycle-bin/empty');
    }
...
function handleRowAction(actionKey, row) {
    if (actionKey === 'edit') {
        router.visit(`/admin/tuition/taxonomies/subjects/${row.id}/edit`);
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="space-y-1">
                    <h1 class="text-2xl font-semibold">{{ filters.trash ? 'Subject Recycle Bin' : 'Subjects' }}</h1>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Link v-if="!filters.trash" href="/admin/tuition/taxonomies/subjects/create" class="rounded-md bg-black px-4 py-2 text-sm text-white">
                        Create Subject
                    </Link>
                </div>
            </div>
            ...
            <div class="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Select v-model="classFilter">
                    ...
                    <SelectContent>
                        ...
                        <SelectItem v-for="schoolClass in schoolClasses" :key="schoolClass.id" :value="String(schoolClass.id)">
                            {{ schoolClass.category_name ? `${schoolClass.name} (${schoolClass.category_name})` : schoolClass.name }}
                        </SelectItem>
                    </SelectContent>
                </Select>
            </div>

            <DataTable :items="items" :columns="columns" empty-text="No subjects found.">

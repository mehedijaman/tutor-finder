...
watch(categoryFilter, (value) => {
    applyFilters({ category_id: value === 'all' ? '' : value, page: 1 });
});
...
function openConfirm(action, row = null, payload = {}) {
    ...
    if (action === 'delete') {
        ...
        confirmDescription.value = 'This will move the class to recycle bin.';
    ...
    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete Class';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/tuition/taxonomies/classes/${row.id}`);
    }

    if (action === 'status' && row) {
        router.patch(`/admin/tuition/taxonomies/classes/${row.id}/status`, {
            status: payload.status,
        });
    }

    if (action === 'restore' && row) {
        router.patch(`/admin/tuition/taxonomies/classes/${row.id}/restore`);
    }

    if (action === 'force-delete' && row) {
        router.delete(`/admin/tuition/taxonomies/classes/${row.id}/force`);
    }

    if (action === 'empty-recycle-bin') {
        router.delete('/admin/tuition/taxonomies/classes/recycle-bin/empty');
    }
...
function handleRowAction(actionKey, row) {
    if (actionKey === 'edit') {
        router.visit(`/admin/tuition/taxonomies/classes/${row.id}/edit`);

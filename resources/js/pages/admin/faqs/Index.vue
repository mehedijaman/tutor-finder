...
const breadcrumbs = [{ title: 'FAQs', href: '/admin/faqs' }];
...
function openConfirm(action, row = null, payload = {}) {
    ...
    if (action === 'delete') {
        confirmTitle.value = 'Delete FAQ';
        confirmDescription.value = 'This will move the FAQ to recycle bin.';
    ...
    if (action === 'status') {
        ...
        confirmTitle.value = isDeactivate ? 'Deactivate FAQ' : 'Activate FAQ';
        confirmDescription.value = isDeactivate
            ? 'This FAQ will no longer appear on the public FAQ page.'
            : 'This FAQ will appear on the public FAQ page.';
    ...
    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete FAQ';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/faqs/${row.id}`);
    }

    if (action === 'status' && row) {
        router.patch(`/admin/faqs/${row.id}/status`, {
            status: payload.status,
        });
    }

    if (action === 'restore' && row) {
        router.patch(`/admin/faqs/${row.id}/restore`);
    }

    if (action === 'force-delete' && row) {
        router.delete(`/admin/faqs/${row.id}/force`);
    }

    if (action === 'empty-recycle-bin') {
        router.delete('/admin/faqs/recycle-bin/empty');
    }
...
function handleRowAction(actionKey, row) {
    if (actionKey === 'edit') {
        router.visit(`/admin/faqs/${row.id}/edit`);

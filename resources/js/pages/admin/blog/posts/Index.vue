...
const breadcrumbs = [{ title: 'Blog Posts', href: '/admin/blog/posts' }];
...
function openConfirm(action, row = null) {
    ...
    if (action === 'delete') {
        confirmTitle.value = 'Delete Post';
        confirmDescription.value = 'This will move the post to recycle bin.';
        confirmLabel.value = 'Delete';
        confirmDestructive.value = true;
    }

    if (action === 'restore') {
        confirmTitle.value = 'Restore Post';
        confirmDescription.value = 'This will restore the post from recycle bin.';
        confirmLabel.value = 'Restore';
    }

    if (action === 'force-delete') {
        confirmTitle.value = 'Permanently Delete Post';
    ...
    if (action === 'empty-recycle-bin') {
        ...
        confirmDescription.value = 'This will permanently remove all trashed posts.';
...
function runConfirmedAction() {
    ...
    if (action === 'delete' && row) {
        router.delete(`/admin/blog/posts/${row.id}`);
    }

    if (action === 'restore' && row) {
        router.patch(`/admin/blog/posts/${row.id}/restore`);
    }

    if (action === 'force-delete' && row) {
        router.delete(`/admin/blog/posts/${row.id}/force`);
    }

    if (action === 'empty-recycle-bin') {
        router.delete('/admin/blog/posts/recycle-bin/empty');
    }
...
function handleRowAction(actionKey, row) {
    if (actionKey === 'edit') {
        router.visit(`/admin/blog/posts/${row.id}/edit`);
        return;
    }
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="space-y-1">
                    <h1 class="text-2xl font-semibold">
                        {{ filters.trash ? 'Post Recycle Bin' : 'Blog Posts' }}
                    </h1>
                ...
                <div class="flex items-center gap-2">
                    <Link
                        :href="filters.trash ? '/admin/blog/posts' : '/admin/blog/posts?trash=1'"
